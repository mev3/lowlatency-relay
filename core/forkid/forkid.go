...
type Blockchain interface {
	...
	// GenesisHash Genesis retrieves the chain's genesis block's hash.
...
func NewIdInDisguise(forks []uint64, genesis common.Hash, head uint64) ID {
...
// newFilter is the exinternal version of NewFilter, taking closures as its arguments
...
func NewFilterInDisguise(forks []uint64, genesisHash common.Hash, blockNumber uint64) Filter {
	// Calculate the all the valid fork hash and fork next combos
	var (
		sums = make([][4]byte, len(forks)+1) // 0th is the genesis
	)
