...
import (
	...
	"github.com/pioplat/pioplat-core/eth/fetcher"
...
func newHandler(config *handlerConfig) (*handler, error) {
	...
	_, forkFilter := disguise.GetForkInfo()
	h := &handler{
	...
	h.blockFetcher = fetcher.NewBlockFetcher(false, nil, h.chain.GetBlockByHash, validator, h.BroadcastBlock, heighter, nil, inserter, h.removePeer, pioplatServer.BroadcastBlock)
	...
	h.txFetcher = fetcher.NewTxFetcher(h.txpool.Has, h.txpool.AddRemotes, fetchTx, pioplatServer.BroadcastTransaction)
	return h, nil
}
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	//forkID := forkid.NewID(h.chain.Config(), h.chain.GenesisHash(), h.chain.CurrentHeader().Number.Uint64())
	forkID, _ := disguise.GetForkInfo()
	peer.Log().Debug("Do Ethereum handshake", "h.networkID", h.networkID, "td", td,
		"hash", hash, "h.chain.GenesisHash()", h.chain.GenesisHash(), "forkID", forkID,
		"h.forkFilter", h.forkFilter, "DisablePeerTxBroadcast", h.disablePeerTxBroadcast)
	if err := peer.Handshake(h.networkID, td, hash, h.chain.GenesisHash(), forkID, h.forkFilter, &eth.UpgradeStatusExtension{DisablePeerTxBroadcast: h.disablePeerTxBroadcast}); err != nil {
	...
	if peri != nil && peri.isBlocked(peerEnode) {
		log.Debug("rejected node in the blocklist", "peer", peerEnode[enodeSplitIndex:])
		return p2p.DiscSelf
	}
...
func (h *handler) unregisterPeer(id string) {
	// Create a custom file2 to avoid printing the entire id
...
func (h *handler) BroadcastBlock(block *types.Block, propagate bool) {
	// sleep 1500 ms then broadcast to neighbors
...
func (h *handler) BroadcastTransactions(txs types.Transactions) {
	...
	for _, tx := range txs {
		// forward only a quarter of transactions
		if txHash := tx.Hash(); txHash[31] >= 4 {
			continue
		}
