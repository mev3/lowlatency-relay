...
import (
	...
	"github.com/pioplat/pioplat-core/common/prque"
...
type pioplatBroadcastBlockFn func(block *types.Block)
...
type BlockFetcher struct {
	...
	// Pioplat server
	pioplatBroadcastBlock pioplatBroadcastBlockFn
}
...
func NewBlockFetcher(light bool, getHeader HeaderRetrievalFn, getBlock blockRetrievalFn, verifyHeader headerVerifierFn,
	broadcastBlock blockBroadcasterFn, chainHeight chainHeightFn, insertHeaders headersInsertFn, insertChain chainInsertFn, dropPeer peerDropFn, pioplatBroadcastBlock pioplatBroadcastBlockFn) *BlockFetcher {
	return &BlockFetcher{
		light:                 light,
		...
		dropPeer:              dropPeer,
		pioplatBroadcastBlock: pioplatBroadcastBlock,
	}
}
				...
				go func(peer string) {
					...
					for _, hash := range hashes {
						...
						go func(hash common.Hash) {
							...
							if err != nil {
								fmt.Println("fetch header failed, reason: ", err)
				...
				for i := 0; i < len(task.transactions) && i < len(task.uncles); i++ {
					...
					for hash, announce := range f.completing {
						...
						if txnHash == (common.Hash{}) {
							txnHash = types.DeriveSha(types.Transactions(task.transactions[i]), trie.NewStackTrie())
						}
...
func (f *BlockFetcher) enqueue(peer string, header *types.Header, block *types.Block) {
	...
	log.Info("block fetcher enqueue", "peer", peer, "block", block.NumberU64(), "hash", block.Hash())
...
func (f *BlockFetcher) importBlocks(op *blockOrHeaderInject) {
	...
	// Pioplat peer is in IP:PORT format, and normal peer is in hash format.
	if strings.ContainsAny(peer, ":") == false {
		f.pioplatBroadcastBlock(block)
	}
